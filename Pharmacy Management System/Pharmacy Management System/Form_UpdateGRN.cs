...
{
    public partial class Form_UpdateGRN : Form
    {
        ...
        private void Btn_Add_Click(object sender, EventArgs e)
        {
            try
            {
                if (cmb_Medicine.SelectedIndex == -1 || txt_Quantity.Text == "" || txt_UnitPrice.Text == "")
                ...
                else
                {
                    ...
                    dataGridView_GRN.Rows.Add(cmb_GRNID.SelectedValue.ToString(), i, txt_MedicineID.Text, cmb_Medicine.SelectedValue.ToString(), txt_SupID.Text, cmb_Supplier.SelectedValue.ToString(), txt_Quantity.Text, txt_UnitPrice.Text, date_Exp.Value.ToString("yyyy-MM-dd"), txt_Discount.Text, txt_SalePrice.Text, tot);
        ...
        private void btn_UpdateRow_Click(object sender, EventArgs e)
        {
            try
            {
                if (cmb_GRNID.SelectedIndex == -1 || cmb_Medicine.SelectedIndex == -1 || txt_Quantity.Text == "" || txt_UnitPrice.Text == "")
        ...
        private void Btn_ClearFields_Click_1(object sender, EventArgs e)
        {
        ...
        {
            if (cmb_GRNID.SelectedIndex == -1)
            {
                MessageBox.Show(this, "Select GRN ID and try Again", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            ...
            else
            {
                for (int y = 0; y < 3; y++)
                {
                    if (y == 0)
                    {
                        ...
                        {
                            con.Open();
                            cmd = new SqlCommand("SELECT * FROM GRN_Master WHERE GRN_Id = '" + cmb_GRNID.SelectedValue.ToString() + "' ", con);
                            cmd.ExecuteNonQuery();
                            SqlDataReader dr;
                            dr = cmd.ExecuteReader();
                            ...
                            while (dr.Read())
                            {
                                date_GRN.Value = Convert.ToDateTime(dr[1]);
                                txt_InvoiceID.Text = dr[2].ToString();
                                date_Invoice.Value = Convert.ToDateTime(dr[3]);
                                txt_SupID.Text = dr[5].ToString();
                        ...
                        {
                            MessageBox.Show(this, "Select New and try again", "Information", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    ...
                    else if (y == 1)
                    {
                        ...
                        {
                            con.Open();
                            cmd = new SqlCommand("SELECT * FROM Supplier WHERE Sup_Id = '" + txt_SupID.Text + "' ", con);
                            cmd.ExecuteNonQuery();
                            SqlDataReader dr;
                            dr = cmd.ExecuteReader();
                            ...
                            while (dr.Read())
                            {
                                cmb_Supplier.SelectedValue = dr[1].ToString();
                            }
                            con.Close();
                            cmd.Dispose();
                        }
                    ...
                    else if (y == 2)
                    {
                        try
                        {
                            ...
                            cmd = new SqlCommand("SELECT * FROM GRN_Detail WHERE GRN_Id = '" + cmb_GRNID.SelectedValue.ToString() + "' ", con);
                            ...
                            {
                                dataGridView_GRN.Rows.Add(ds.Tables[0].Rows[j]["GRN_Id"].ToString(), Convert.ToInt32(ds.Tables[0].Rows[j]["Grid_Order_No"]), ds.Tables[0].Rows[j]["Med_Id"].ToString(), ds.Tables[0].Rows[j]["Med_Name"].ToString(), ds.Tables[0].Rows[j]["Sup_Id"].ToString(), ds.Tables[0].Rows[j]["Sup_Name"].ToString(), Convert.ToDecimal(ds.Tables[0].Rows[j]["Qty"]), Convert.ToDecimal(ds.Tables[0].Rows[j]["Unit_Price"]), ds.Tables[0].Rows[j]["Exp_Date"].ToString(), Convert.ToDecimal(ds.Tables[0].Rows[j]["Discount"]), Convert.ToDecimal(ds.Tables[0].Rows[j]["Sale_Price"]), Convert.ToDecimal(ds.Tables[0].Rows[j]["Total"]));
                                TopMost = TopMost;
                                Show();
                            }
        ...
        private void Btn_Print_Click(object sender, EventArgs e)//Need to edit
        {
            if (cmb_GRNID.SelectedIndex == -1 || txt_InvoiceID.Text == "" || cmb_Supplier.SelectedIndex == -1 || cmb_Medicine.SelectedIndex == -1 || txt_SupID.Text == "" || txt_MedicineID.Text == "" || txt_UnitPrice.Text == "" || txt_Quantity.Text == "" || txt_Discount.Text == "")
            {
                MessageBox.Show(this, "Missing Informations. Complete and try Again", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private void PrintDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)//Need to edit
        {
            ...
            string grnNo = "GRN No : " + Convert.ToString(cmb_GRNID.SelectedValue.ToString());
        ...
        private void Cmb_GRNID_SelectedIndexChanged(object sender, EventArgs e)
        {
            
        }

        private void Btn_UpdateGRN_Click(object sender, EventArgs e)
        {
                ...
                try
                {
                    for (int y = 0; y < 2; y++)
                    {
                        if (y == 0)
                        {
                            ...
                            cmd = new SqlCommand("UPDATE GRN_Master SET GRN_Date = @GRNDATE, Invoice_ID = @INVID, Invoice_Date = @INVDATE, GRN_Amount = @SUBTOTAL, Sup_Id = @SUPID WHERE GRN_Id = @GRNID", con);

                            cmd.Parameters.AddWithValue("@GRNID", cmb_GRNID.SelectedValue);
            ...
            {
                cmd = new SqlCommand("SELECT Grid_Order_No FROM GRN_Detail WHERE GRN_Id = @GRNID", con);
                {
                    con.Open();
                    ...
                    if (j == 0)
                        MessageBox.Show(this, "Does not exists", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);

                    else if (j == 1)
                        MessageBox.Show(this, "Exists", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
